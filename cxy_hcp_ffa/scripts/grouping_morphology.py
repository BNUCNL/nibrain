...
def pre_ANOVA_3factors(meas_name):
    ...
    src_file = pjoin(anal_dir, 'structure/rois_v3_{hemi}_{meas_name}.pkl')
...
def pre_ANOVA_3factors_mix(meas_name):
    """
    准备好3因素混合设计方差分析需要的数据。
    被试间因子：group
    ...
    trg_file = pjoin(work_dir, f'rois-v3_{meas_name}_preANOVA-3factor-mix.csv')

    hemi2data = {}
    hemi2gids = {}
    for hemi in hemis:
        hemi2data[hemi] = pkl.load(open(src_file.format(hemi=hemi,
                                   meas_name=meas_name), 'rb'))
