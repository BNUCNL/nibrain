...
resting_img = CiftiReader(
    '/nfs/e1/HCPD/fmriresults01/HCD0008117_V1_MR/MNINonLinear/Results/rfMRI_REST1_PA/rfMRI_REST1_PA_Atlas_MSMAll_hp0_clean.dtseries.nii')
...
for subject in ['HCD0008117_V1_MR']:
    ...
    #
    alff_falff = np.zeros((17853, 2))
    fc = np.zeros((17853, 91282))
    ...
    # ALFF
    alff_falff_mean = alff_falff / 4
    # save
    alff_falff_vol = np.zeros((91, 109, 91, alff_falff_mean.shape[1]))
    for col in np.arange(alff_falff_mean.shape[1]):
        alff_falff_vol[cb_index_matrix[:, 0], cb_index_matrix[:, 1], cb_index_matrix[:, 2], col] = alff_falff_mean[:,
                                                                                                   col]
    save2nifti(os.path.join(save_dir, 'rfMRI_REST_cerebellum_alff_falff.nii.gz'), alff_falff_vol, affine=affine_matrix,
               header=None)
    ...
    save2nifti(os.path.join(save_dir, 'rfMRI_REST_cerebellum_gbc_L.nii.gz'), gbc_L_vol, affine=affine_matrix,
               header=None)
    # right hemisphere
    gbc_R = compute_gbc(fc_mean, target_R)
    gbc_R_vol = np.zeros((91, 109, 91, gbc_R.shape[1]))
    for gbc_R_col in np.arange(gbc_R.shape[1]):
        gbc_R_vol[cb_index_matrix[:, 0], cb_index_matrix[:, 1], cb_index_matrix[:, 2], gbc_R_col] = gbc_R[:, gbc_R_col]
    save2nifti(os.path.join(save_dir, 'rfMRI_REST_cerebellum_gbc_R.nii.gz'), gbc_R_vol, affine=affine_matrix,
               header=None)
