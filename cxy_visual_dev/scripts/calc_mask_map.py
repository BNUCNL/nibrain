...
def make_mask4():
    """
    依据各ROI在HCPY-M+T_MMP-vis3-R_zscore1_PCA-subj中的PC1的值排序分成6级
    """
    n_level = 6
    fpath = pjoin(anal_dir, 'ROI_scalar/ROI_scalar1_MMP-vis3-R.csv')
    atlas = Atlas('HCP-MMP')
    out_file = pjoin(work_dir, 'HCPY-M+T_MMP-vis3-R_zscore1_PCA-subj_6level-ROI.dlabel.nii')

    df = pd.read_csv(fpath, index_col=0)
    idx = 'mean_stru-C1'
    rois_list = [[] for _ in range(n_level)]
    rois_list[0].append('R_V1')
    rois_list[1].append('R_V2')
...
def make_mask5():
    """
    依据各ROI在HCPY-M+T_MMP-vis3-R_zscore1_PCA-subj中的PC1的值排序分成5级
    """
    n_level = 5
    fpath = pjoin(anal_dir, 'ROI_scalar/ROI_scalar1_MMP-vis3-R.csv')
    atlas = Atlas('HCP-MMP')
    out_file = pjoin(work_dir, 'HCPY-M+T_MMP-vis3-R_zscore1_PCA-subj_5level-ROI.dlabel.nii')
...
def make_mask7(N):
    ...
    out_file = pjoin(work_dir, f'HCPY-M+T_MMP-vis3-R_zscore1_PCA-subj_mask7-N{N}.dlabel.nii')
